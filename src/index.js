...
function onSearch(e) {
  ...
  newsApiService.query = e.currentTarget.elements.query.value; // введенное слово для поиска

  if (newsApiService.query === '') {
    return alert('Введи что-то нормальное');
  }

  loadMoreBtn.show(); // отрисовка кнопки "показать еще"
  newsApiService.resetPage(); //
  clearArticlesContainer(); // что бы статьи появлялись сразу под поиском
  fetchArticles(); // результат запроса
}
